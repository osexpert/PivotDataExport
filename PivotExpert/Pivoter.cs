...
namespace PivotExpert
{
	public class Pivoter<TRow> where TRow : class // notnull
	{
		...
		public Pivoter(IEnumerable<Field> fields, IEnumerable<TRow> list, ITypedList typedList) : this(fields, list, typedList.GetItemProperties(null!))
		{

		}

		public Pivoter(IEnumerable<Field> fields, IEnumerable<TRow> list, PropertyDescriptorCollection pdc)
		{
		...
		private List<List<Group<TRow>>> GroupRows(IEnumerable<Field> fields, bool sort = false)
		{
			...
			lastGroups.Add(new Group<TRow> { Rows = _list, IsRoot = true });
      ...
      "/ItemType:Baby Food/SalesChannel:Offline":
      {
		 ...
		 * 
		 * */
		public Table<KeyValueClass<TRow>> GetTableFastIntersect_DictArrNEsted()
		{
			var s1d = GetTableFastIntersect();
		...
		private Table<KeyValueClass<TRow>> NextDictNested(Pivoter<TRow>.Step1Data s1d)
		{
		...
		private Group<TRow> CloneColGroups(Group<TRow> lg, Dictionary<(Group<TRow>?, object?), Group<TRow>>[] lookupGroups)
		{
	...
	.OrderBy(f => f.SortIndex).ToArray();
		...
		private string GetGrpNamePath(Group<TRow> lastColGroup, Dictionary<Group<TRow>, string> dict)
		{
